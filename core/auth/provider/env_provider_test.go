// Copyright 2022 FE Technologies Co.,Ltd.
//
...
import (
	...
	"github.com/fecloud-sdk/fecloud-sdk-go/core/auth/global"
...
func TestEnvCredentialProvider_GetCredentials3(t *testing.T) {
	...
	err := os.Setenv("FECLOUD_SDK_AK", MockAk)
	assert.Nil(t, err)
	err = os.Setenv("FECLOUD_SDK_SK", MockSk)
	assert.Nil(t, err)
	p := BasicCredentialEnvProvider()
	credentials, err := p.GetCredentials()
	assert.Nil(t, err)
	...
	p = BasicCredentialEnvProvider()
	assert.Nil(t, err)
	err = os.Setenv("FECLOUD_SDK_PROJECT_ID", MockProjectId)
	assert.Nil(t, err)
	credentials, err = p.GetCredentials()
	assert.Nil(t, err)
...
func TestEnvCredentialProvider_GetCredentials5(t *testing.T) {
	...
	assert.Nil(t, err)
	p := BasicCredentialEnvProvider()
	err = os.Setenv("FECLOUD_SDK_IDP_ID", MockIdpId)
	assert.Nil(t, err)
	err = os.Setenv("FECLOUD_SDK_ID_TOKEN_FILE", MockIdTokenFile)
	assert.Nil(t, err)
	err = os.Setenv("FECLOUD_SDK_PROJECT_ID", MockProjectId)
	assert.Nil(t, err)
	credentials, err := p.GetCredentials()
...
func resetEnv() error {
	envs := []string{
		"FECLOUD_SDK_AK",
		"FECLOUD_SDK_SK",
		"FECLOUD_SDK_PROJECT_ID",
		"FECLOUD_SDK_DOMAIN_ID",
		"FECLOUD_SDK_CREDENTIAL_TYPE",
		"FECLOUD_SDK_IDP_ID",
		"FECLOUD_SDK_ID_TOKEN_FILE",
	}
